...
<p id="description_2"></p>
  ...
  <div id="container">
    ...
    <div id="main">
      <div id="content_left">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td><select name="region" id="region">
                    <option value="(Upstream500)">Upstream500</option>
                    <option value="(Upstream1000)">Upstream1000</option>
                    <option value="(Gene body)">Gene body</option>
                    <option value="(Downstream500)">Downstream500</option>
                    <option value="(Downstream1000)">Downstream1000</option>
                  </select>
                </td>
              </tr>
              <tr>
        ...
        <table width="700" border="1" cellpadding="0" cellspacing="0" align="center" bordercolor="#CCCCCC" style="display: none;font-size:10px;text-align:center">
          <tbody>
            <tr><td>Gene</td><td>DNA methylation level</td><td>Shannon entropy</td><td>No. of segments</td><td>Sample</td><td>Plot all</td></tr>
      ...
      <div id="content_right">
        ...
        <div class="item_box">
          ...
          <p style="font-size:13px">The landscapes of heterogeneity in DNA methylation are shown in the UCSC Genome Browser.</p>
        </div>
        <div class="item_box">
    ...
    <div id="footer">
        <div style="float:left; padding-left:40px;"> Copyright 2018 &copy;<a href="http://qianlab.genetics.ac.cn/"> QianLab</a> </div>
        <div id="madeby"></div>
      </div>
  </div>
</body>
...
if(isset($_POST)){
  ...
  $mydownload = "SELECT * INTO OUTFILE '/tmp/QuertResult$index.csv' FIELDS TERMINATED BY '\t' LINES TERMINATED BY '\n'  FROM `$species` where `Gene_ID` IN ('" . implode("','", $gene) . "')";
  ...
  $title = "Gene_ID DNA methylation level(Upstream1000) DNA methylation level(Upstream500)  DNA methylation level(gene body)  DNA methylation level(Downstream500)  DNA methylation level(Downstream1000) DNA methylation entropy(Upstream1000) DNA methylation entropy(Upstream500)  DNA methylation entropy(gene body)  DNA methylation entropy(Downstream500)  DNA methylation entropy(Downstream1000) Number of DNA methylation regions(Upstream1000) Number of DNA methylation regions(Upstream500)  Number of DNA methylation regions(gene body)  Number of DNA methylation regions(Downstream500)  Number of DNA methylation regions(Downstream1000) GEO";
  exec("sed -i '1i $title' /opt/lampp/htdocs/website/HeteroMeth/tmp/QuertResult$index.csv");
  //判断返回结果是否为空
  ...
  else
  {
	...
	echo "$('.style2').html('Query Result of $species at: $region');";
    ...
    echo "<div height='5px' width='650'><p style='font-size:20px'>Download the query ,click <a href='tmp/QuertResult$index.csv' download='QueryResult.txt'>here</a>";
    echo '");';

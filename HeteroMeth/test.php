...
if(isset($_POST)){
  ...
  $mydownload = "SELECT *  FROM `$species` where `Gene_ID` IN ('" . implode("','", $gene) . "')";
  //$mydownload = "SELECT *  FROM `$species` where `Gene_ID` IN ('" . implode("','", $gene) . "')";
  ...
  $mydatabase_download=$mysqli->query($myquery);
 ...
 // }
  ...
  {
	  $filename = "/opt/lampp/htdocs/website/HeteroMeth/tmp/QuertResult$index.$output_format";
	  $out = fopen($filename,'w');
	  echo $out;
	  fputcsv($out,$title,",",chr(127));
	  while ($row=$mydatabase_download->fetch_row()) 
	  {
		  fputcsv($out,array_values($row),",",chr(127));
          echo $row;
	  }
  }
  else
  {
	  ...
	  $out = fopen($filename,'w');
	  fputcsv($out,$title,"\t",chr(127));
	  while ($row=$mydatabase_download->fetch_row()) fputcsv($out,array_values($row),"\t",chr(127));
  }
  //判断返回结果是否为空
